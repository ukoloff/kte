...
FOR ii=1 TO 15 STEP 1
   DO case
       CASE  ii=1
         ...
         n1=n-npred
         m.cur_kte=VAL(SUBSTR(m.input_inst_info,n1,1)) 
         npred=n  		             
       CASE  ii=2
...
DO CASE
   ...
   OTHERWISE MESSAGEBOX("Вариант для расчета не определен",0," ")
ENDCASE
...
IF m.instr_OK=.f.
		   IF  m.instr_OK=.f.   &&& Если резец не найден
		    ...
		    IF n>=1              
             ...
             MESSAGEBOX("Сверло с заданным диаметром и длиной обработки не найдено. Выбираем сверло с максимальным диаметром 12 мм.",0," ")
		     m.instr_OK=.t.
		     m.Ar=6
		    endif   		    
endif
...
*  Проверка по мощности и усилиям не проводится
